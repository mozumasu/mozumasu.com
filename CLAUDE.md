...
- `water.js` は 3 パス。水面はビューポート大のテクスチャ (FBO) に画面座標で 1 回描き、ドキュメント全体に
  重ねた `#water` には表示範囲の帯にそのテクスチャを貼って、`.glass` 要素ごとの矩形でガラス板を描く。
  板はページ座標なので要素から遅れず、板の中も外も同じテクスチャを同じスクロール位置で参照する。
  この分担は崩さない: 板を固定キャンバスに描くと要素から 1 フレーム遅れ (8e17eac)、板の中だけ画面座標で
  別キャンバスに描くと再描画の合間にずれて揺れる (bef7c83)。どちらも試して戻した
- 要素側に `filter` や `mask` を足すと GPU 合成で四角い影が出る。見た目の効果はシェーダー側に寄せる
- `?t=` の静止画はテクスチャの範囲 (最初の 1 画面 + 上下 15%) しか正しくない。下のほうを撮るときは
  対象より上を消した一時コピーで撮る (headless-render スキルの手順)

...
- `water.js` は 3 パス。表示範囲の帯の水面をページ座標のままテクスチャに描き (スマホは `ws` = 0.5 の低解像度)、
  ドキュメント全体に重ねた `#water` の同じ帯にそれを貼り、`.glass` 要素ごとの矩形でガラス板を描く (キャンバスは
  スマホ 0.7)。水面の解像度と板の解像度を別々に決められるのがこの構成の目的。
  要素側に `filter` や `mask` を足すと GPU 合成で四角い影が出る。見た目の効果はシェーダー側に寄せる
- **水面は必ずページ座標で描く**。画面座標の水面を使う案は 3 度試して全部戻した: 板を固定キャンバスに描くと
  要素から 1 フレーム遅れる (8e17eac)、板だけページに貼ると板の中の水面が再描画の合間にずれて揺れる (bef7c83)、
  水面をテクスチャに描いて毎フレーム `scrollY` でページに貼り直すとコンポジタとの数 px のずれで小刻みに震える
  (4a37fc3)。JS が読む `scrollY` はコンポジタの位置と一致しないので、画面座標のものをページに貼る設計は全滅する。
